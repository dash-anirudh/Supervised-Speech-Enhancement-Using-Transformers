...
class EnhancementModel(nn.Module):
    def __init__(self, hop=160, n_fft=512, win=400):
        ...
        self.deconv4 = nn.ConvTranspose2d(32, 16, kernel_size=(3, 4), stride=(1, 2), padding=(1, 1))
    ...
    def forward(self, x):
        ...
        x_conv6 = self.activation(self.bn6(self.conv6(x_conv5)))
        x_conv7 = self.activation(self.bn7(self.conv7(x_conv6)))
        x_conv7 = x_conv7 + x_conv5
        
        x_deconv1 = self.activation(self.bn_deconv1(self.deconv1(x_conv7)))
        x_deconv1 = x_deconv1 + x_conv4
        
        x_deconv2 = self.activation(self.bn_deconv2(self.deconv2(x_deconv1)))
        x_deconv2 = x_deconv2 + x_conv3
        
        x_deconv3 = self.activation(self.bn_deconv3(self.deconv3(x_deconv2)))
        x_deconv3 = x_deconv3 + x_conv2
        
        x_deconv4 = self.activation(self.bn_deconv4(self.deconv4(x_deconv3)))
        x_deconv4 = x_deconv4 + x_conv1
        
        x_deconv5 = F.tanh(self.bn_deconv5(self.deconv5(x_deconv4)))
        ...
        mask_spec = mask_spec.squeeze(1)
        
        enhanced_real = enhanced_spec[:, 0, :, :]
        enhanced_imag = enhanced_spec[:, 1, :, :]
        enhanced_cmplx_spec = torch.complex(enhanced_real, enhanced_imag)
        enhanced_cmplx_spec = enhanced_cmplx_spec.squeeze(1)
...
if __name__ == '__main__':
    ...
    print(summary(model, input_size=(1, valid_length_time(32160)), device='cpu'))
